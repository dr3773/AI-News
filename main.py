...
# Токен бота: берём из TELEGRAM_BOT_TOKEN (как в Render),
# BOT_TOKEN / TOKEN — запасные варианты.
TOKEN = (
...
ADMIN_ID = os.environ.get("ADMIN_ID")  # опционально, для ошибок
...
def build_body_text(title: str, summary: str) -> str:
    ...
    Если описания нет, или оно почти повторяет заголовок
    (отличается только хвостом вида " - сайт.ком" / доменом),
    возвращаем ПУСТУЮ строку — тогда под заголовком ничего не будет.
    """
    ...
    def almost_same(a: str, b: str) -> bool:
        """Почти одинаковые строки (учитываем маленький 'хвостик')."""
        if not a or not b:
            return False
        ...
        # а хвост не длиннее 15 символов (например: " - ixbt.com")
    ...
    if almost_same(t, s):
        # summary по сути дублирует заголовок — не выводим
        return ""
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_chat.send_message(
        ...
        "Он регулярно публикует до 5 свежих новостей за один цикл без спама и дублей текста."
    )
