...
def build_ai_comment(title: str, summary: Optional[str]) -> str:
    ...
    (без фразы «что это значит» в самом тексте)
    """
    category = classify_category(title, summary)

    if category == "модели и технологии ИИ":
        return (
            "Разработчики усиливают архитектуры и качество моделей. "
            "Это открывает путь к более сложным сценариям применения ИИ в ближайшие месяцы."
        )
    if category == "стартапы и инвестиции в ИИ":
        return (
            "Рынок ИИ-стартапов остаётся активным: капитал продолжает идти в компании, "
            "которые предлагают практичные и масштабируемые решения."
        )
    if category == "регулирование и безопасность ИИ":
        return (
            "Регуляторы формируют правила для использования ИИ. "
            "Это влияет и на скорость внедрения технологий, и на требования к компаниям-разработчикам."
        )
    if category == "прикладные продукты и сервисы на базе ИИ":
        return (
            "Фокус смещается в сторону реальных продуктов. "
            "Такие новости показывают, как ИИ превращается из эксперимента в привычный инструмент."
        )
    if category == "ИИ в медицине":
        return (
            "ИИ в медицине развивается от пилотных проектов к реальной клинической практике. "
            "Это может улучшить диагностику, маршрутизацию пациентов и поддержку врачебных решений."
        )
    if category == "ИИ в образовании":
        return (
            "ИИ меняет подход к обучению, делая его более персонализированным. "
            "Подобные инициативы могут ускорить освоение сложных навыков."
        )

    return (
        "Новость вписывается в общий тренд ускоренного развития ИИ. "
        "Такие события помогают понять, какие направления будут ключевыми для индустрии в ближайшем будущем."
    )
...
async def send_single_news(context: ContextTypes.DEFAULT_TYPE, item: Dict) -> None:
    ...
    - источник-ссылка
    - ИИ-комментарий (без фразы «что это значит»)
    ...
    # ИИ-комментарий
    parts.append("")
    parts.append(f"🤖 ИИ-комментарий: {html_escape(ai_comment, quote=False)}")
...
async def send_evening_digest(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for i, item in enumerate(DAILY_ITEMS, start=1):
        ...
        lines.append(f"   🤖 ИИ-комментарий: {safe_ai_comment}")
        lines.append("")
